...
Plugin URI: https://themes.themegoods.com/grandrestaurant/landing
...
if($is_verified_envato_purchase_code) {
	...
	*/
	...
	$tg_default_currency = get_theme_mod('tg_default_currency', '$');
	...
	*/
	...
	$postmetas = 
		array (
			...
			'menus' => array(
				...
				array("section" => "Menu Option", "id" => "menu_price_currency", "type" => "text", "title" => "Price Currency", "description" => "Enter menu price currency ex. $", "default" => $tg_default_currency),
		    	...
		    	array("section" => "Menu Option", "id" => "menu_highlight_title", "type" => "text", "title" => "Highlight Title", "description" => "Enter hight title ex. Chef Recommended", "default" => ''),
	...
	function new_meta_box() {
				...
				foreach ( $postmeta as $postmeta_key => $each_meta ) {
					...
					if ($meta_type == 'checkbox') {
						...
						if(isset($postmeta[$postmeta_key]['sample']))
						{
							...
							if(isset($each_meta['default']) && !empty($each_meta['default']))
							{
								$current_value = $each_meta['default'];
							}
							
							$post_value = get_post_meta($post->ID, $meta_id, true);
							if(!empty($post_value))
							{
						...
						else
						{
							...
							echo "<p><input type='text' name='$meta_id' id='$meta_id' class='' value='".esc_Attr($current_value)."' style='width:99%' /></p>";
						}
					}
		...
		if($_GET['post_type'] == 'menus')
		{
			...
			echo '<a id="menu_size_add_row" class="button" data-target="menu_size_sortable" href="javascript:;">'.__( 'Add New', PLUGINDOMAIN ).'</a>';
