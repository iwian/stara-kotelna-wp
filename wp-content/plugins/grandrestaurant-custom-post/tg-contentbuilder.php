	...
	if($align=='left')
	{
		...
		if(!empty($menu_arr) && is_array($menu_arr))
		{
			foreach($menu_arr as $key => $menu_item)
			{
		        ...
		        {
		        	$return_html.= '
		            	<span class="menu_price" style="background:'.esc_attr($bgcolor).';">'.grandrestaurant_format_price($menu_price[0], $menu_ID).'</span>';
		        }
				...
				if(!empty($menu_size_item_title_arr) && is_array($menu_size_item_title_arr))
				{
					...
					foreach($menu_size_item_title_arr as $key => $menu_size_item_title_item)
					{
						...
						{
							$return_html.= '<span class="menu_price size" style="background:'.esc_attr($bgcolor).';">'.grandrestaurant_format_price($menu_size_item_price_arr[$key], $menu_ID).'</span>';
						}
					}
				}
	...
	if(!empty($menu_arr) && is_array($menu_arr))
	{
	    foreach($menu_arr as $key => $menu_item)
	    {
	        ...
	        if(isset($menu_price[0]))
	        {
	        	...
	        	$return_html.= '">'.grandrestaurant_format_price($menu_price[0], $menu_ID).'</span>';
	        }
			...
			if(!empty($menu_size_item_title_arr) && is_array($menu_size_item_title_arr))
			{
				...
				foreach($menu_size_item_title_arr as $key => $menu_size_item_title_item)
				{
					...
					if(isset($menu_size_item_price_arr[$key]) && !empty($menu_size_item_price_arr[$key]))
					{
						$return_html.= '<span class="menu_price size" style="background:'.esc_attr($bgcolor).';color:'.esc_attr($fontcolor).';">'.grandrestaurant_format_price($menu_size_item_price_arr[$key], $menu_ID).'</span>';
					}
				}
			}
...
function ppb_menu_classic_func($atts, $content) {
	...
	if(!empty($menu_arr) && is_array($menu_arr))
	{
	    foreach($menu_arr as $key => $menu_item)
	    {
		    ...
		    if(isset($menu_price[0]))
		    {
		        	$return_html.= '<span class="menu_price">'.grandrestaurant_format_price($menu_price[0], $menu_ID).'</span>';
		    }
			...
			if(!empty($menu_size_item_title_arr) && is_array($menu_size_item_title_arr))
			{
				...
				foreach($menu_size_item_title_arr as $key => $menu_size_item_title_item)
				{
					...
					if(isset($menu_size_item_price_arr[$key]) && !empty($menu_size_item_price_arr[$key]))
					{
						$return_html.= '<span class="menu_price size">'.grandrestaurant_format_price($menu_size_item_price_arr[$key], $menu_ID).'</span>';
					}
				}
			}
...
function ppb_team_column_func($atts, $content) {
	...
	if(!empty($team_arr) && is_array($team_arr))
	{
		...
		foreach($team_arr as $key => $member)
		{
			...
			if(has_post_thumbnail($member_ID, 'full'))
			{
			    $image_id = get_post_thumbnail_id($member_ID);
			    
			    //Check if has new defined post thumbnail
				if(has_image_size('grandrestaurant-album-grid'))
				{
					$small_image_url = wp_get_attachment_image_src($image_id, 'grandrestaurant-album-grid', true);	
				}
				else
				{
            ...
            if(!empty($member_facebook) OR !empty($member_twitter) OR !empty($member_google) OR !empty($member_linkedin))
			{
			    ...
			    {
			        $return_html.= '<li><a title="'.esc_attr($member->post_title).' on Twitter" target="_blank" class="tooltip" href="https://twitter.com/'.$member_twitter.'" '.$social_font_color.'><i class="fab fa-twitter"></i></a></li>';
			    }
	 
			    if(!empty($member_facebook))
			    {
			        $return_html.= '<li><a title="'.esc_attr($member->post_title).' on Facebook" target="_blank" class="tooltip" href="https://facebook.com/'.$member_facebook.'" '.$social_font_color.'><i class="fab fa-facebook"></i></a></li>';
			    }
			        
			    if(!empty($member_linkedin))
			    {
			        $return_html.= '<li><a title="'.esc_attr($member->post_title).' on Linkedin" target="_blank" class="tooltip" href="'.$member_linkedin.'" '.$social_font_color.'><i class="fab fa-linkedin"></i></a></li>';
			    }
	...
	if(!empty($team_arr) && is_array($team_arr))
	{
		...
		foreach($team_arr as $key => $member)
		{
            ...
            if(!empty($member_facebook) OR !empty($member_twitter) OR !empty($member_google) OR !empty($member_linkedin))
			{
			    ...
			    if(!empty($member_twitter))
			    {
			        $return_html.= '<li><a title="'.esc_attr($member->post_title).' on Twitter" target="_blank" class="tooltip" href="http://twitter.com/'.$member_twitter.'" '.$social_font_color.'><i class="fab fa-twitter"></i></a></li>';
			    }
	 
			    if(!empty($member_facebook))
			    {
			        $return_html.= '<li><a title="'.esc_attr($member->post_title).' on Facebook" target="_blank" class="tooltip" href="http://facebook.com/'.$member_facebook.'" '.$social_font_color.'><i class="fab fa-facebook"></i></a></li>';
			    }
...
function ppb_contact_map_func($atts, $content) {
    ...
    if(is_array($pp_contact_form) && !empty($pp_contact_form))
    {
	    ...
	    <label for="gdpr" class="gdpr_label">'.esc_html__('I consent to ', 'grandrestaurant-custom-post' ).get_bloginfo('name').esc_html__(' collecting my details through this form.', 'grandrestaurant-custom-post' ).'</label>';
	   	}
	...
	//Display Map
	$return_html.= '<div class="one_half_bg nopadding" style="height:100vh">';
	$return_html.= '<div id="map'.$custom_id.'" class="map_shortcode_wrapper" style="width:100%;height:100vh"';
	
	if(!empty($zoom)) {
		$return_html.= 'data-zoom="'.esc_attr($zoom).'" ';
	}
	
	if(!empty($type)) {
		$return_html.= 'data-type="'.esc_attr($type).'" ';
	}
