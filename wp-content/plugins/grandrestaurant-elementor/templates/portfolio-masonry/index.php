...
?>
<div class="grandrestaurant-portfolio-filter-wrapper">
		<a class="filter-tag-btn active" href="javascript:;" data-rel="all" data-cols="<?php echo esc_attr($settings['columns']['size']); ?>"><?php echo __( 'All', 'grandrestaurant-elementor' ); ?></a>
	<?php
		foreach ( $filterable_tags as $filterable_tag ) 
		{
	?>
		<a class="filter-tag-btn" href="javascript:;" data-rel="<?php echo grandrestaurant_sanitize_title($filterable_tag); ?>" data-cols="<?php echo esc_attr($settings['columns']['size']); ?>"><?php echo esc_html($filterable_tag); ?></a>
	<?php
		}
	?>
</div>
...
?>
<div class="portfolio-masonry-content-wrapper grandrestaurant-gallery-grid-content-wrapper do-masonry masonry-classic portfolio_masonry layout-<?php echo esc_attr($column_class); ?>" data-cols="<?php echo esc_attr($settings['columns']['size']); ?>">
<?php		
		$animation_class = '';
		...
		foreach ( $slides as $slide ) 
		{
			...
			if(is_array($slide_tags_arr) && count($slide_tags_arr) > 1)
			{
				...
				{
					$slide_tags.= grandrestaurant_sanitize_title(ltrim($slide_tag)).' ';
				}
			}
			else
			{
				$slide_tags.= grandrestaurant_sanitize_title(ltrim($slide['slide_tag']));
			}
