...
<?php
	...
	if(!empty($slides))
	{
		...
		$column_class = 1;
		$thumb_image_name = 'grandrestaurant-video-grid';
...
<?php		
		...
		foreach ( $slides as $slide ) 
		{
			...
			if(is_array($slide_tags_arr) && count($slide_tags_arr) > 1)
			{
				...
				{
					$slide_tags.= grandrestaurant_sanitize_title($slide_tag).' ';
				}
			}
			else
			{
				$slide_tags.= grandrestaurant_sanitize_title($slide['slide_tag']);
			}
