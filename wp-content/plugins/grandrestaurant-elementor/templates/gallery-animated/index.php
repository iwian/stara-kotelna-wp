<?php
	...
	{
		$images = grandrestaurant_get_gallery_images($settings['gallery_id'], $settings['sort']);
	}
	else
	{
	...
	{
		$images = grandrestaurant_get_gallery_images($post->ID, 'drag');
	}
	
	if(!empty($images))
	{
		$thumb_image_name = 'grandrestaurant-gallery-thumbnail';
		if(isset($settings['image_dimension']) && !empty($settings['image_dimension']))
		{
			$thumb_image_name = $settings['image_dimension'];
		}
?>
<div class="grandrestaurant-gallery-animated-content-wrapper <?php echo esc_attr($thumb_image_name); ?> ri-grid ri-grid-size-2 ri-shadow" data-rows="<?php echo esc_attr($settings['rows']['size']); ?>" data-columns="<?php echo esc_attr($settings['columns']['size']); ?>" data-animation="<?php echo esc_attr($settings['animation_type']); ?>" data-animation-speed="<?php echo esc_attr($settings['animation_speed']['size']); ?>" data-interval="<?php echo esc_attr($settings['interval_time']['size']); ?>">
	<ul>
	<?php				
...
?>
		<li>
			<a class="grandrestaurant_gallery_lightbox" href="<?php echo esc_url($thumb_image_url[0]); ?>">
